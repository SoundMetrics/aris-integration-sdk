...
namespace SoundMetrics.HID.Windows
{
    public static partial class Joystick
    {
        ...
        internal static bool GetJoystickPosition(
            uint joystickId,
            out ValueTuple<uint, JoyInfoEx> posInfo)
        {
            ...
            try
            {
                ...
                if (result == 0)
                {
                    posInfo =
                        (joystickId,
                        (JoyInfoEx)Marshal.PtrToStructure(pinned, typeof(JoyInfoEx)));
                    return true;
                }
                else
                {
                    posInfo = new ValueTuple<uint, JoyInfoEx>();
                    return false;
                }
            }
            finally
